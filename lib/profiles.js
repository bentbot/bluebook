...
// 	Feeds:  			Feed prototype functions 	[ Defined in /lib/feed.js ]
**/
		...
		if ( profile ) {
			...
			profile.comparePassword(password, function (err, isMatch) {
				...
				if ( isMatch === true ) {
					...
					// Fill the result variable with the cookie and profile data.
					// But first, make sure to zero-out the user's password.
					profile.password = 0;
					result = {
						code: 200,
						cookie: cookie,
	...
	if (request) {
		...
		Profile.prototype.getCodename(request, function (err, codename) {
			...
			newUser.save(function (err, responce) {
				...
				// If the user is already in the system, try logging in.
				if (err.code == 11000) {
					
					// E11000 means there is a duplicate key in the database.
					Profile.prototype.login(request.email, request.password, function (err, result) {
